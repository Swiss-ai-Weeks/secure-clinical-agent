...
- `03-identity.sql`: `identity.users (user_id opaque, role, department, credential_level, active)`; `identity.sessions (session_hash = sha256(cookie token), user_id, created_at, last_seen_at, expires_at, absolute_expires_at, auth_level ∈ {1,2}, on_duty, self_patient_id without FK, sandbox_id, revoked_at)`; `identity.run_tokens (jti, session_hash, subject, actor, audience, issued_at, expires_at, revoked_at)`. The session token itself is never stored (ASVS V7). No names, no credentials, no consents.
...
| Delegated tokens | RFC 8693, RFC 9068, RFC 8725 | `sub` + `act.sub`, narrow `aud`, short `exp`, `jti` replay cache, session-bound revocation |
