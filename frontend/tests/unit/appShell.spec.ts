...
describe('AppShell workspaces', () => {
  it('shows a human-readable access level for each persona', async () => {
    await renderShell(sessionMe('attending', ['labs', 'ask'], {
      user_id: 'u_chen',
    ...
    expect(labels).toEqual(expect.arrayContaining([
      'Dr. Sarah Chen · Privileged · Attending',
      'Tomas Lindqvist · Limited · Dietary',
      'Maria Santos · Limited · Family',
      'Priya Nair · Licensed · Research',
      'Nurse Alex Rivera · Licensed · Care team',
      'Dr. James Okafor · Privileged · Consultant'
    ]));
    expect(screen.getByText('Privileged · Attending')).toBeInTheDocument();
    expect(labels.join(' ')).not.toMatch(/u_chen|u_nair|p_|panels|Clinical|Board-certified|credential_level/);
