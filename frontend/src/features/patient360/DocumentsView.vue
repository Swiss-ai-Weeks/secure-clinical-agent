<template>
  <section class="documents"><header><p>Patient 360</p><h1>Documents</h1><label class="upload" for="upload-document">Upload document<input id="upload-document" aria-label="Upload document" type="file" @change="upload" /></label></header><ClinicalCard title="Patient documents"><div class="documents__list"><article v-for="document in patient?.documents" :key="document.id"><div><h2>{{ document.title }}</h2><p>{{ document.type }} · {{ document.date }} · {{ document.source }}</p></div><div><StatusPill :label="document.processingState" :tone="document.processingState === 'processed' ? 'success' : 'warning'" /><span>Uploaded by {{ document.uploadedBy }}</span></div></article></div></ClinicalCard><ClinicalCard v-if="facts.length" title="Information detected" :ai="true"><div class="facts"><article v-for="fact in facts" :key="fact.id"><div><strong>{{ fact.label }}</strong><p>{{ fact.value }}</p><span v-if="fact.status === 'accepted'">Accepted into review queue</span><span v-else-if="fact.status !== 'pending'">{{ fact.status }}</span></div><div class="fact-actions"><button type="button" @click="decide(fact.id, 'accepted')">Accept</button><button type="button" @click="decide(fact.id, 'edited')">Edit</button><button type="button" @click="decide(fact.id, 'rejected')">Reject</button></div></article></div></ClinicalCard></section>
</template>
