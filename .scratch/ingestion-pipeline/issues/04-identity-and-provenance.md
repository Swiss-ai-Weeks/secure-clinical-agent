...
Resolved 2026-09-17 and amended after the user reviewed the imported main schema: keep birth year only, allow raw non-patient source resource IDs temporarily, and replace the demo PostgreSQL volume. The earlier full-DOB/restricted-source-only proposals above are discussion history; this amended resolution is the current contract.
...
- **Traceability:** retain restricted patient mappings and detailed provenance for the ingestion worker and authorized operator. As an explicit temporary exception, use the imported schema's raw non-patient `ResourceType/id` values in clinical `source_id` columns for upserts. Patient identity remains opaque; application results use opaque citations. Do not copy raw source IDs into embedding requests, ordinary logs, or application citations. [Move source resource identifiers into restricted provenance](12-restrict-source-resource-identifiers.md) tracks removing this exception. Preserve mapping state across restarts and back it up; dev-mode OpenBao is not the durable store.
- **Dates and demographics:** keep birth year only in prepared structured records, including year-only `Patient.birthDate` JSONB. This supersedes full synthetic DOB retention. Do not add full birth dates to notes, embedding requests, or vector metadata. Preserve synthetic clinical event dates/times and intervals; read-time age display follows the imported schema's year-based calculation and 90+ cap. Detailed narrative age handling remains in the note policy ticket. Restricted immutable FHIR source artifacts retain their original dates.
- **Demo database bootstrap:** the user authorized replacing the current PostgreSQL data volume to initialize the imported schema. Back up the existing database first. This does not authorize deleting other stores or raw source artifacts, automatically resetting the database on worker retries, or expanding the worker's database privileges.
...
- Patient, encounter, document, and source-resource identities are distinct. The temporary clinical schema also retains raw non-patient source IDs as upsert keys; its lack of namespace columns limits this initial importer to the agreed source namespace, with smoke/seed sharing it. Do not import unrelated source namespaces into those keys until the provenance follow-up resolves isolation. Track source versions/content digests and preparation-policy versions separately from logical identity; changed content does not silently create a different patient. Chunk identity and replacement rules remain in the chunking/vector decisions.
...
Required implementation checks: stable keys across smoke/seed and replay; namespace/resource identity boundaries; invalid/cross-patient reference rejection; registry restart and backup/restore behavior; traceability from opaque citations; raw source IDs limited to the temporary internal clinical upsert fields and restricted provenance; no source IDs in embedding requests, application citations, or logs; year-only prepared birth dates and preserved clinical event dates. These are adapter requirements, not evidence of completed ingestion.

The identity/provenance decision is complete; the adapter, registry, sanitization, and seed publication are not implemented by this resolution. [Choose the FHIR-to-PostgreSQL projection and update rules](05-fhir-projection.md) owns the safe field projection and update rules; [Choose note de-identification and chunk boundaries](07-deid-and-chunking.md) owns recognizers, note handling, and chunking under this policy. Runtime schema replacement evidence is recorded separately in the [schema integration checkpoint](../../../backend/ingestion/schema-integration.md).
