...
## Immediate next step: identity and sanitized artifacts

Begin with [Choose the first cohort and ingestion success criteria](../../.scratch/ingestion-pipeline/issues/03-cohort-and-success.md) and [Define patient identity, linkage, and source provenance](../../.scratch/ingestion-pipeline/issues/04-identity-and-provenance.md). Generator identification is resolved; the remaining clinical/product choices have not been silently approved.

1. Confirm the demo defaults or record alternatives: Massachusetts, English upstream encounter notes, reference/end date 2026-09-01, ten-year exported history, and Patient/Condition/Observation projection. Define a few source-grounded clinical examples for later search evaluation. Decide whether a separately labelled adversarial evaluation corpus is wanted.
2. Specify stable opaque patient/document/resource identity, the linkage namespace, date treatment, and restricted provenance storage. Keep the same patient key across structured rows, notes, chunks, and grants. Do not use raw FHIR identifiers as the public identity contract without resolving this decision.
...
5. Test deterministic identity, cross-patient rejection, source lineage, sanitization failure handling, clinical-value preservation, and absence of raw identifiers in prepared artifacts and logs.
...
| --- | --- | --- |
| Structured storage | Add migrations and stable source-resource upsert keys for the agreed safe Patient/Condition/Observation projection. | Correct patient links, codes/values/units, and replay without duplicates; defined changed/deleted-source behavior. |
...
> Read backend/ingestion/NEXT-STEPS.md and the linked decision tickets. Continue from the verified Synthea source worker toward sanitized, authorized vector search. First resolve the remaining cohort and identity/de-identification decisions, then implement and test the preparation stage. Preserve source artifacts and existing services. Do not report vector ingestion or retrieval complete until real model/store and authorization checks pass.

Use the [ordered implementation backlog](issues.md) and [decision map](../../.scratch/ingestion-pipeline/map.md) to record progress. GitHub implementation issues were not closed during this session; keep incomplete acceptance criteria visible. The unrelated local product documents, assets, and staged document moves are not part of the ingestion publication.
