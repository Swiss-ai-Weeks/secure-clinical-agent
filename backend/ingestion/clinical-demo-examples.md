...
Preserve the association between these source fixtures and future opaque IDs when implementing preparation. After sanitization, recheck that diagnosis/care context, numeric values, units, and patient/encounter linkage survive before using the examples as end-to-end acceptance evidence. Identifier removal, date treatment, safe field projection, grants, and publication are still unresolved downstream contracts.
