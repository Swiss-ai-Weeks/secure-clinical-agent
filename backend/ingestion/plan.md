...
| [FHIR initialization SQL](../deploy/patient360/sql/fhir/01-init.sql) | Main adds `clinical`, `identity`, and `audit` schemas, top-level JSONB allowlists, source uniqueness, component observations, and patient-scoped foreign keys. The user adopted birth year only and temporary internal raw source IDs. The demo PostgreSQL volume was backed up, rebuilt, and validated; see [schema integration](schema-integration.md). |
