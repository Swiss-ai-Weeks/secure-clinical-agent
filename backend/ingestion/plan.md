...
| --- | --- |
| [Patient360 Compose](../deploy/patient360/compose.yaml) | PostgreSQL 16, Qdrant 1.13.6, OpenFGA, immudb, OpenBao, MinIO, and local Nemotron embedding are defined. No Synthea, Presidio, ingestion worker, retrieval policy service, or Patient360 reranker is defined. The `ingest` profile selects imaging models. |
| [FHIR initialization SQL](../deploy/patient360/sql/fhir/01-init.sql) | `patients`, `conditions`, `observations`, `studies` exist. JSONB `resource` is unrestricted despite the no-names comment. Conditions/observations use generated UUIDs with no source-resource uniqueness. Existing database volumes need explicit migrations. |
