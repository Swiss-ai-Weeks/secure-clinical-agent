...
| [backend/ingestion/](backend/ingestion/) | Synthetic ingestion plan, research, and future pipeline implementation |
...
The backend guide lists commands for starting stores and model services. The Compose `ingest` profile selects imaging models; the new synthetic ingestion worker is still being planned in [backend/ingestion/plan.md](backend/ingestion/plan.md).
